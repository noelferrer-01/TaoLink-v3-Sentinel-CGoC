...
| **B8** | The Employees "Assign to detachment…" bulk action was still an `alert()` placeholder (`"Bulk-assign N selected — wire in Phase 9.5"`). Originally deferred to Slice 3 polish, but Slice 2 contract Step 6 explicitly walks the clerk through Employees → multi-select → bulk-assign modal — so this IS criterion-mandated for Slice 2 close-out, not polish. Same TODO existed for "Change status…" which is genuinely out-of-scope (per-target rules differ; ships per-individual on the detail page; bulk version lands in Slice 3+). | new [`app/(admin)/employees/actions.ts`](../../app/%28admin%29/employees/actions.ts), new [`app/(admin)/employees/bulk-assign-modal.tsx`](../../app/%28admin%29/employees/bulk-assign-modal.tsx), new shared [`components/modal-shell.tsx`](../../components/modal-shell.tsx), [`app/(admin)/employees/employees-list-body.tsx`](../../app/%28admin%29/employees/employees-list-body.tsx), [`app/(admin)/employees/page.tsx`](../../app/%28admin%29/employees/page.tsx) | New `bulkAssignAction` server action → `assignments.bulkAssign` (per-row failures return as `errors`, batch never aborts). New `BulkAssignModal` mirrors the Bulk Transfer modal on `/assignments`: typeahead detachment + start date + Confirm/Cancel. After submit, a result panel shows "N assigned, M failed" with friendly per-row reasons ("Aquino, Rosa: this guard already has an active assignment — end the previous one first"). The shared modal chrome moved to `components/modal-shell.tsx` since this is the third caller — `assignments-list-body.tsx` now imports it too instead of declaring inline. Browser-verified the partial-failure path end-to-end (2 selected, both already assigned → "0 assigned. 2 rows failed." with named errors). "Change status…" placeholder removed from the bar; footer hint updated. |
| **B9** | "Download BIR 2316 (PDF)" click didn't save anything to `~/Downloads` — Noel verified by inspecting the folder after a click. Server response was correct (200, `application/pdf`, `Content-Disposition: attachment; filename="2316-CG-10001-2026.pdf"`), link element was correct (`<a download="2316-2026.pdf" href="…">`, no `target="_blank"`), but Chrome still treated the response as a navigation and rendered the PDF inline in the same tab rather than downloading. Known Chrome behavior — `Content-Disposition: attachment` is not enough on some Chrome configurations when the link click is a top-level navigation to an `application/pdf` URL on the same origin. | [`app/(admin)/exports/bir-picker.tsx`](../../app/%28admin%29/exports/bir-picker.tsx) | Switched from `<a download href>` to a `<button onClick>` that fetches the PDF as a Blob, creates a `URL.createObjectURL(blob)`, programmatically clicks a hidden anchor with `download="2316-CG-XXXXX-YYYY.pdf"`, then revokes the object URL. The browser never sees a navigation to a `.pdf` URL — it only sees a click on an anchor whose `href` is a `blob:` URL with `download`, which Chrome reliably saves to ~/Downloads regardless of PDF-viewer settings. Filename prefers the server's `Content-Disposition` `filename=` (so it includes the employee code) and falls back to year-only. Loading state ("Generating PDF…"), error surface, and field hint updated to say "Saves to your computer's Downloads folder." Verified via Playwright that the blob fetch returns 200 + 6435-byte `application/pdf`. |
