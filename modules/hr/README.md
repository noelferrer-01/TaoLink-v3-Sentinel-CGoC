...
| `hr.listReadinessIssues` | `({ armedOnly?, limit?, offset?, today? }) => Promise<{ rows: ReadinessIssue[], total }>` | **Slice 3b licence readiness radar** (lives in hr because it joins `employees` with person credentials and uses no recruitment code — the radar UI page sits under `/recruitment` by design). Active (non-terminated) guards diffed against the required *credential* set per `isArmedPost` (null → unarmed). Per required type: a held VALID one clears it (LTOPF still emits the `unverified` "firearm link unverified" caveat — ADR 0018); else the soonest `expiring`; else the most severe present state (`revoked` > `expired` > `pending`); else `missing`. Per-credential renewal windows via `CRED_WINDOW_DAYS` are authoritative (no user window control). Computes in-app over all active guards then paginates — see the 3b done-sweep perf note. `today` is injectable for tests; defaults to Manila today. |

Labels/types: `READINESS_KIND_LABELS` + the `ReadinessKind`, `ReadinessIssue`, `ReadinessQuery` types.
...
- **Env:** `DATABASE_URL`.
- **Modules:** `@/modules/persons` (mints/links the Person that owns each employee's identity; `searchEmployees` joins `persons` for the name; Slice 3b `listReadinessIssues` also uses `listCredentialsForPersons`, `READINESS_CRED_SET`, `CRED_WINDOW_DAYS`, `deriveCredState`), `@/modules/audit` (writes audit rows on every mutation), `@/modules/events` (publishes `hr.employee.created`, `hr.employee.status_changed`, and `hr.employee.updated`). Audit/events are non-fatal — a failure there does NOT roll back the HR insert; the Person mint, by contrast, is in the same transaction as the employee insert and DOES roll back together.
