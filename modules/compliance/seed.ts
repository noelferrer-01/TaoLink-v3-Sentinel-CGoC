...
export async function seedComplianceRates(opts: { effectiveDate?: string } = {}) {
  ...
  await db.transaction(async (tx) => {
    ...
    await tx.delete(sssBrackets).where(eq(sssBrackets.effectiveDate, effectiveDate));
    await tx.delete(philhealthConfig).where(eq(philhealthConfig.effectiveDate, effectiveDate));
    await tx.delete(pagibigConfig).where(eq(pagibigConfig.effectiveDate, effectiveDate));
    await tx.delete(wtaxBrackets).where(eq(wtaxBrackets.effectiveDate, effectiveDate));
    ...
    await tx.insert(sssBrackets).values(sssRows);

    // 2. PhilHealth — RA 11223. Circular 2019-0009. 5% rate, floor ₱10k, ceiling ₱100k.
    await tx.insert(philhealthConfig).values([{
    ...
    await tx.insert(pagibigConfig).values([{
    ...
    await tx.insert(wtaxBrackets).values([
